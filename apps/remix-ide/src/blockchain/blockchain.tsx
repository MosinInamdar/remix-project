...
export class Blockchain extends Plugin {
  ...
  onActivation() {
    ...
    this.on("manager", "pluginActivated", (plugin) => {
      if (
        plugin &&
        plugin.name &&
        (plugin.name.startsWith("injected") || plugin.name === "walletconnect")
      ) {
  ...
  getCurrentProvider() {
    ...
    if (provider && provider.startsWith("injected"))
      return this.providers["injected"];
  ...
  calculateFee(gas, gasPrice, unit?) {
    return (
      toBigInt(gas) *
      toBigInt(toWei(gasPrice.toString(10) as string, unit || "gwei"))
    );
  }
  ...
  changeExecutionContext(context, confirmCb, infoCb, cb) {
    if (context.context === "item-another-chain") {
      this.call("manager", "activatePlugin", "environmentExplorer").then(() =>
        this.call("tabs", "focus", "environmentExplorer")
      );
    } else {
  ...
  runOrCallContractMethod(
    ...
    txFormat.buildData(
      ...
      (error, data) => {
        ...
        const useCall =
          funABI.stateMutability === "view" ||
          funABI.stateMutability === "pure";
        this.runTx(
  ...
  addProvider(provider: Provider) {
    if (this.pinnedProviders.includes(provider.name))
  ...
  async loadContext(context: string) {
    ...
    if (saveEvmState) {
      const contextExists = await this.call(
        "fileManager",
        "exists",
        `.states/${context}/state.json`
      );

      if (contextExists) {
        const stateDb = await this.call(
          "fileManager",
          "readFile",
          `.states/${context}/state.json`
        );
    ...
    web3Runner.event.register("transactionBroadcasted", (txhash) => {
      this.executionContext.detectNetwork((error, network) => {
        ...
        if (viewEtherScanLink) {
          if (network.name === "Apothem" || network.name === "XDCMain") {
            this.call(
              "terminal",
              "logHtml",
      ...
      this.executionContext.detectNetwork((error, network) => {
        ...
        this.txRunner.rawRun(
          ...
          async (error, result) => {
            ...
            try {
              if (this.executionContext.isVM()) {
                const execResult = await this.web3().remix.getExecutionResultFromSimulator(
        ...
        try {
          this.txRunner.rawRun(
            ...
            async (error, result) => {
              ...
              if (isVM && tx.useCall) {
                try {
                  result.transactionHash = await this.web3().remix.getHashFromTagBySimulator(
                    timestamp
                  );
                } catch (e) {
    ...
    try {
      ...
      */
      ...
      if (isVM) {
        if (!tx.useCall && this.config.get("settings/save-evm-state")) {
          try {
            ...
            this.call(
              "fileManager",
              "writeFile",
              `.states/${this.executionContext.getProvider()}/state.json`,
              state
            );
          } catch (e) {
            console.error(e);
        ...
        const hhlogs = await this.web3().remix.getHHLogsForTx(
        ...
        execResult = await this.web3().remix.getExecutionResultFromSimulator(
          txResult.transactionHash
        );
