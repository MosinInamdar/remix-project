...
export const Renderer = ({ message, opt = {}, plugin }: RendererProps) => {
  ...
  const [selectedAI, setSelectedAI] = useState(localStorage.getItem("AIModel"));

  useEffect(() => {
    const interval = setInterval(() => {
      const newAIModel = localStorage.getItem("AIModel");
      if (newAIModel !== selectedAI) {
        setSelectedAI(newAIModel);
      }
  ...
  const askGtp = async () => {
    try {
      const content = await plugin.call(
        "fileManager",
        "readFile",
        editorOptions.errFile
      );
      const message = intl.formatMessage(
        { id: "solidity.openaigptMessage" },
        { content, messageText }
      );
