...
export function RemixUIStatusBar({ statusBarPlugin }: RemixUIStatusBarProps) {
  ...
  useEffect(() => {
    ...
    async function getScamAlerts() {
      const response = await axios.get(
        "https://raw.githubusercontent.com/remix-project-org/remix-dynamics/main/ide/scam-alerts.json",
        { signal }
      );
  ...
  const lightAiUp = async () => {
    const aiActive = await statusBarPlugin.call(
      "settings",
      "get",
      "settings/copilot/suggest/activate"
    );
    if (!aiActive) return;
