...
export function UniversalDappUI(props: UdappProps) {
  ...
  useEffect(() => {
    ...
    if (props.instance.address) {
      if (prefix == "0x") {
        ...
        let address =
          (props.instance.address.slice(0, 2) === "xdc" ? "" : "") +
          // @ts-ignore
          props.instance.address.toString("hex");
        address = ethJSUtil.toChecksumAddress(address);
        setAddress(address);
  ...
  const sendData = () => {
    ...
    if (!receive && !fallback)
      return setLlIError(intl.formatMessage({ id: "udapp.llIError6" }));
    ...
    if (!args.funcABI)
      return setLlIError(intl.formatMessage({ id: "udapp.llIError7" }));
  ...
  const unsavePinnedContract = async () => {
    await props.plugin.call(
      "fileManager",
      "remove",
      `.deploys/pinned-contracts/${props.plugin.REACT_API.chainId}/${props.instance.address}.json`
    );
  };
  ...
  const pinContract = async () => {
    const workspace = await props.plugin.call(
      "filePanel",
      "getCurrentWorkspace"
    );
    const objToSave = {
      ...
      filePath:
        ...
        `${workspace.name}/${props.instance.contractData.contract.file}`,
      pinnedAt: Date.now(),
    };
    await props.plugin.call(
      "fileManager",
    ...
    _paq.push([
      ...
      "udapp",
      "pinContracts",
      `pinned at ${props.plugin.REACT_API.chainId}`,
    ]);
  ...
  const runTransaction = (
    ...
    if (props.isPinnedContract)
      _paq.push(["trackEvent", "udapp", "pinContracts", "interactWithPinned"]);
    const functionName =
  ...
  const label = (key: string | number, value: string) => {
    value = value.replace(/address: \w{2}/, `address: ${prefix}`);
    return (
  ...
  return (
    <div
      ...
      data-id={
        props.isPinnedContract
          ? `pinnedInstance${address}`
          : `unpinnedInstance${address}`
      }
    >
      <div className="udapp_title pb-0 alert alert-secondary">
        ...
        <div className="input-group udapp_nameNbuts">
          <div className="udapp_titleText input-group-prepend">
            {props.isPinnedContract ? (
              <CustomTooltip
                ...
                tooltipId="udapp_udappUnpinTooltip"
                tooltipText={
                  props.isPinnedContract
                    ? `Contract: ${
                        props.instance.name
                      },  Address: ${address}, Pinned at:  ${new Date(
                        props.instance.pinnedAt
                      ).toLocaleString()}`
                    : ""
                }
              >
          ...
          <div className="btn" style={{ padding: "0.15rem" }}>
            <CopyToClipboard
              tip={intl.formatMessage({ id: "udapp.copyAddress" })}
              content={address}
              direction={"top"}
            />
          </div>
          {props.isPinnedContract ? (
            <div
              className="btn"
              style={{ padding: "0.15rem", marginLeft: "-0.5rem" }}
            >
              <CustomTooltip
                placement="top"
                tooltipClasses="text-nowrap"
                tooltipId="udapp_udappUnpinTooltip"
                tooltipText={<FormattedMessage id="udapp.tooltipTextUnpin" />}
              >
                <i
                  className="fas fa-thumbtack p-2"
                  aria-hidden="true"
                  data-id="universalDappUiUdappUnpin"
                  onClick={remove}
                ></i>
              </CustomTooltip>
            </div>
            ...
            >
              <CustomTooltip
                placement="top"
                tooltipClasses="text-nowrap"
                tooltipId="udapp_udappPinTooltip"
                tooltipText={<FormattedMessage id="udapp.tooltipTextPin" />}
              >
                <i
                  className="far fa-thumbtack p-2"
                  aria-hidden="true"
                  data-id="universalDappUiUdappPin"
                  onClick={pinContract}
                ></i>
              </CustomTooltip>
            </div>
        ...
        {props.isPinnedContract ? (
          ...
          >
            <CustomTooltip
              placement="top"
              tooltipClasses="text-nowrap"
              tooltipId="udapp_udappDeleteTooltip"
              tooltipText={<FormattedMessage id="udapp.tooltipTextDelete" />}
            >
              <i
                className="far fa-trash p-2"
                aria-hidden="true"
                data-id="universalDappUiUdappDelete"
                onClick={deletePinnedContract}
              ></i>
            </CustomTooltip>
          </div>
        ) : (
          ...
          >
            <CustomTooltip
              ...
              tooltipText={<FormattedMessage id="udapp.tooltipTextRemove" />}
            >
              <i
      ...
      >
        <div className="udapp_contractActionsContainer">
          <div
            className="d-flex flex-row justify-content-between align-items-center pb-2"
            data-id="instanceContractBal"
          >
            ...
            <div className="d-flex align-self-center">
              {props.exEnvironment &&
                props.exEnvironment.startsWith("injected") && (
                  <CustomTooltip
                    placement="top"
                    tooltipClasses="text-nowrap"
                    tooltipId="udapp_udappEditTooltip"
                    tooltipText={
                      <FormattedMessage id="udapp.tooltipTextEdit" />
                    }
                  >
                    <i
          ...
          {props.isPinnedContract && props.instance.filePath ? (
            <div
              className="d-flex"
              data-id="instanceContractFilePath"
            ...
            >
              {
                ...
                props.solcVersion.canReceive === false ? (
                  <a
                    href={`https://solidity.readthedocs.io/en/v${props.solcVersion.version}/contracts.html`}
                    target="_blank"
                    rel="noreferrer"
                  >
                  ...
                  <a
                    href={`https://solidity.readthedocs.io/en/v${props.solcVersion.version}/contracts.html#receive-ether-function`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <i aria-hidden="true" className="fas fa-info my-2 mr-1"></i>
